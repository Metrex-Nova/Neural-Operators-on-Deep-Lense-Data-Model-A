...
class ConvBranch(nn.Module):
    """
    CNN encoder for the branch net -- replaces the flat-MLP branch used in
    Vanilla/Stacked DeepONet. Convolutions respect kappa's 2D spatial
    structure (local subhalo features, edges) instead of treating all 4096
    pixels as an unordered flat vector.
    """
    def __init__(self, res=64, out_dim=128):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(1, 32, 3, stride=2, padding=1), nn.GELU(),   # res/2
            nn.Conv2d(32, 64, 3, stride=2, padding=1), nn.GELU(),  # res/4
            nn.Conv2d(64, 128, 3, stride=2, padding=1), nn.GELU(), # res/8
            nn.AdaptiveAvgPool2d(4),
        )
        flat_dim = 128 * 4 * 4
        self.fc = nn.Sequential(nn.Linear(flat_dim, 256), nn.GELU(), nn.Linear(256, out_dim))
...
class ConvDeepONet(nn.Module):
    """
    Conv-DeepONet: branch net is a CNN over kappa (spatial structure
    preserved), trunk net is the usual coordinate-MLP. One shared branch
    encoding split across output channels (similar combination style to
    Vanilla DeepONet, but with a convolutional branch instead of an MLP).
    """
    def __init__(self, res=64, out_channels=3, p=128, trunk_hidden=128):
        super().__init__()
        self.res = res
        self.out_channels = out_channels
        self.p = p

        self.branch = ConvBranch(res=res, out_dim=out_channels * p)
    ...
    def forward(self, x):
        ...
        branch_out = self.branch(x)  # (B, out_channels*p)
...
model = ConvDeepONet(res=RES, out_channels=3, p=128, trunk_hidden=128).to(device)
psf_blur = LearnablePSFBlur(kernel_size=9, init_sigma=1.0).to(device)
print("Conv-DeepONet params:", sum(p.numel() for p in model.parameters()))
...
for epoch in range(EPOCHS):
    ...
    if epoch >= WARMUP_EPOCHS:
        if val_img < best_val:
            best_val = val_img
            patience_ctr = 0
            torch.save({"model": model.state_dict(), "psf_blur": psf_blur.state_dict()}, "best_conv_deeponet.pt")
        else:
            patience_ctr += 1
...
ckpt = torch.load("best_conv_deeponet.pt")
...
print("WDM test | Conv-DeepONet image rel_l2:", test_img, "| alpha rel_l2:", test_alpha)
...
for idx in sample_indices:
    ...
    axes[0, 1].imshow(img_pred, cmap="viridis"); axes[0, 1].set_title("predicted (ConvDeepONet + PSF)")

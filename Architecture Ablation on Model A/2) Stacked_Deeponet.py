...
class SingleDeepONet(nn.Module):
    """
    One independent branch+trunk pair, producing ONE scalar output field.
    This is the building block of "Stacked" DeepONet: instead of one shared
    network split across output channels (Vanilla), each output channel gets
    its own dedicated branch and trunk network with no shared weights.
    """
    ...
    def forward(self, x_flat, coords):
        branch_out = self.branch(x_flat)      # (B, p)
        trunk_out = self.trunk(coords)         # (N, p)
        out = torch.einsum("bp,np->bn", branch_out, trunk_out) + self.bias
        return out  # (B, N)

class StackedDeepONet(nn.Module):
    """
    Stacked DeepONet: one fully independent SingleDeepONet per output
    channel (alpha_x, alpha_y, lens_light), each with its own branch and
    trunk network -- no shared weights between outputs, unlike Vanilla
    DeepONet's single branch/trunk split across channels.
    """
    def __init__(self, res=64, out_channels=3, p=128, branch_hidden=256, trunk_hidden=128):
        ...
        self.nets = nn.ModuleList([
            SingleDeepONet(res=res, p=p, branch_hidden=branch_hidden, trunk_hidden=trunk_hidden)
    ...
    def forward(self, x):
        ...
        for net in self.nets:
            out = net(x_flat, self.coords)  # (B, N)
            outs.append(out)

        out = torch.stack(outs, dim=1)  # (B, out_channels, N)
...
model = StackedDeepONet(res=RES, out_channels=3, p=128, branch_hidden=256, trunk_hidden=128).to(device)
psf_blur = LearnablePSFBlur(kernel_size=9, init_sigma=1.0).to(device)
print("Stacked DeepONet params:", sum(p.numel() for p in model.parameters()))
...
for epoch in range(EPOCHS):
    ...
    if epoch >= WARMUP_EPOCHS:
        if val_img < best_val:
            best_val = val_img
            patience_ctr = 0
            torch.save({"model": model.state_dict(), "psf_blur": psf_blur.state_dict()}, "best_stacked_deeponet.pt")
        else:
            patience_ctr += 1
...
ckpt = torch.load("best_stacked_deeponet.pt")
...
print("WDM test | Stacked DeepONet image rel_l2:", test_img, "| alpha rel_l2:", test_alpha)
...
for idx in sample_indices:
    ...
    axes[0, 1].imshow(img_pred, cmap="viridis"); axes[0, 1].set_title("predicted (StackedDeepONet + PSF)")

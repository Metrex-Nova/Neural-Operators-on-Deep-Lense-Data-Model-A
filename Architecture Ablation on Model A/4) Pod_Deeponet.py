...
# ---- Compute fixed POD basis from training alpha fields (normalized space) ----
# POD-DeepONet's trunk is NOT learned -- it's a fixed set of orthogonal spatial
# basis functions (from SVD/PCA on real training fields), which the branch net
# then only needs to predict coefficients for. This differs from Vanilla/Stacked/
# Conv-DeepONet, where the trunk itself is a learned coordinate-MLP.
N_POD_MODES = 32

ax_train_norm = normalize(ax_tr, alpha_x_mean, alpha_x_std).reshape(len(ax_tr), -1)  # (N, H*W)
ay_train_norm = normalize(ay_tr, alpha_y_mean, alpha_y_std).reshape(len(ay_tr), -1)

pod_mean_x = ax_train_norm.mean(dim=0)  # (H*W,)
pod_mean_y = ay_train_norm.mean(dim=0)

centered_x = ax_train_norm - pod_mean_x
centered_y = ay_train_norm - pod_mean_y
...
pod_basis_x = V_x  # (H*W, N_POD_MODES) -- fixed, orthonormal spatial basis functions
pod_basis_y = V_y

explained_x = (S_x ** 2).sum() / (centered_x ** 2).sum()
explained_y = (S_y ** 2).sum() / (centered_y ** 2).sum()
print(f"POD basis computed: {N_POD_MODES} modes | alpha_x variance explained: {explained_x:.4f} | alpha_y: {explained_y:.4f}")

pod_mean_x = pod_mean_x.to(device)
pod_mean_y = pod_mean_y.to(device)
pod_basis_x = pod_basis_x.to(device)
pod_basis_y = pod_basis_y.to(device)

# %% Cell 5 

class ConvBranch(nn.Module):
    """CNN encoder for the branch net -- takes kappa, outputs a latent vector."""
...
class SingleDeepONet(nn.Module):
    """Standard learned branch+trunk pair, used only for lens_light (no POD
    ground truth available for that channel since it's an unsupervised residual)."""
    def __init__(self, res=64, p=128, branch_hidden=256, trunk_hidden=128):
        ...
        self.trunk = nn.Sequential(
            ...
            nn.Linear(trunk_hidden, p), nn.GELU()
        )
        self.bias = nn.Parameter(torch.zeros(1))

    def forward(self, x_flat, coords):
        branch_out = self.branch(x_flat)
        trunk_out = self.trunk(coords)
        return torch.einsum("bp,np->bn", branch_out, trunk_out) + self.bias

class PODDeepONet(nn.Module):
    """
    POD-DeepONet: for alpha_x and alpha_y, the trunk is REPLACED by a fixed
    POD basis (computed via SVD on real training fields) -- the branch net
    only needs to learn coefficients along these known, data-driven directions,
    rather than learning the basis functions themselves from scratch like a
    standard trunk-MLP does. lens_light keeps a standard learned branch+trunk
    since no ground-truth field exists to compute a POD basis from.
    """
    def __init__(self, res, pod_basis_x, pod_mean_x, pod_basis_y, pod_mean_y, n_modes=32, branch_dim=128):
        super().__init__()
        self.res = res
        self.n_modes = n_modes

        self.register_buffer("pod_basis_x", pod_basis_x)  # (H*W, n_modes)
        self.register_buffer("pod_mean_x", pod_mean_x)      # (H*W,)
        self.register_buffer("pod_basis_y", pod_basis_y)
        self.register_buffer("pod_mean_y", pod_mean_y)

        self.branch = ConvBranch(out_dim=branch_dim)
        self.coeff_head = nn.Sequential(
            nn.Linear(branch_dim, branch_dim), nn.GELU(),
            nn.Linear(branch_dim, 2 * n_modes)  # coefficients for alpha_x and alpha_y
        )

        self.lens_light_net = SingleDeepONet(res=res, p=128)
    ...
    def forward(self, x):
        ...
        coeffs = self.coeff_head(branch_feat)  # (B, 2*n_modes)
        coeffs_x, coeffs_y = coeffs[:, :self.n_modes], coeffs[:, self.n_modes:]

        # reconstruct field = mean + coeffs @ basis^T (fixed, data-driven basis)
        alpha_x_flat = self.pod_mean_x.unsqueeze(0) + coeffs_x @ self.pod_basis_x.T
        alpha_y_flat = self.pod_mean_y.unsqueeze(0) + coeffs_y @ self.pod_basis_y.T
        alpha_x = alpha_x_flat.view(B, H, W)
        alpha_y = alpha_y_flat.view(B, H, W)

        x_flat = x.reshape(B, -1)
        lens_light_flat = self.lens_light_net(x_flat, self.coords)
        lens_light = lens_light_flat.view(B, H, W)

        out = torch.stack([alpha_x, alpha_y, lens_light], dim=1)
        return out
...
model = PODDeepONet(RES, pod_basis_x, pod_mean_x, pod_basis_y, pod_mean_y, n_modes=N_POD_MODES, branch_dim=128).to(device)
psf_blur = LearnablePSFBlur(kernel_size=9, init_sigma=1.0).to(device)
print("POD-DeepONet params:", sum(p.numel() for p in model.parameters()))
...
for epoch in range(EPOCHS):
    ...
    if epoch >= WARMUP_EPOCHS:
        if val_img < best_val:
            best_val = val_img
            patience_ctr = 0
            torch.save({"model": model.state_dict(), "psf_blur": psf_blur.state_dict()}, "best_pod_deeponet.pt")
        else:
            patience_ctr += 1
...
ckpt = torch.load("best_pod_deeponet.pt")
...
print("WDM test | POD-DeepONet image rel_l2:", test_img, "| alpha rel_l2:", test_alpha)
...
for idx in sample_indices:
    ...
    axes[0, 1].imshow(img_pred, cmap="viridis"); axes[0, 1].set_title("predicted (POD-DeepONet + PSF)")

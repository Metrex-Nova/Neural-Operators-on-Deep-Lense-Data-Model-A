...
class FourierFeatureEncoding(nn.Module):
    """
    Random Fourier feature encoding for coordinates: maps (x,y) -> 
    [sin(2*pi*B*coord), cos(2*pi*B*coord)] using a FIXED random projection
    matrix B. This lets a coordinate-MLP represent high-frequency spatial
    variation much more easily than feeding in raw (x,y) directly -- a
    well-known fix for the "coordinate networks are low-frequency biased"
    problem, directly relevant to resolving fine subhalo-scale structure.
    """
    ...
    def forward(self, coords):
        proj = 2 * np.pi * coords @ self.B  # (N, n_freqs)
...
class FourierDeepONet(nn.Module):
    """
    Fourier-DeepONet: same branch/combination structure as Conv-DeepONet, but
    the trunk net's input coordinates are first passed through a fixed random
    Fourier feature encoding before the trunk-MLP, instead of using raw (x,y).
    """
    def __init__(self, res=64, out_channels=3, p=128, n_freqs=64, fourier_scale=10.0, trunk_hidden=128):
        super().__init__()
        self.res = res
        ...
        self.fourier_enc = FourierFeatureEncoding(in_dim=2, n_freqs=n_freqs, scale=fourier_scale)
        self.trunk = nn.Sequential(
            nn.Linear(self.fourier_enc.out_dim, trunk_hidden), nn.GELU(),
    ...
    def forward(self, x):
        ...
        coord_feats = self.fourier_enc(self.coords)  # (N, 2*n_freqs)
        trunk_out = self.trunk(coord_feats)  # (N, out_channels*p)
        N = trunk_out.shape[0]
...
model = FourierDeepONet(res=RES, out_channels=3, p=128, n_freqs=64, fourier_scale=10.0, trunk_hidden=128).to(device)
psf_blur = LearnablePSFBlur(kernel_size=9, init_sigma=1.0).to(device)
print("Fourier-DeepONet params:", sum(p.numel() for p in model.parameters()))
...
for epoch in range(EPOCHS):
    ...
    if epoch >= WARMUP_EPOCHS:
        if val_img < best_val:
            best_val = val_img
            patience_ctr = 0
            torch.save({"model": model.state_dict(), "psf_blur": psf_blur.state_dict()}, "best_fourier_deeponet.pt")
        else:
            patience_ctr += 1
...
# %% Cell 7
              ...
              ckpt = torch.load("best_fourier_deeponet.pt")
...
print("WDM test | Fourier-DeepONet image rel_l2:", test_img, "| alpha rel_l2:", test_alpha)
...
for idx in sample_indices:
    ...
    axes[0, 1].imshow(img_pred, cmap="viridis"); axes[0, 1].set_title("predicted (Fourier-DeepONet + PSF)")
